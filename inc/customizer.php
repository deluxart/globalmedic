 ...
 */
function Ember_customize_register( $wp_customize ) {
	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial( 'blogname', array(
			...
			'render_callback' => 'Ember_customize_partial_blogname',
		) );
		$wp_customize->selective_refresh->add_partial( 'blogdescription', array(
			'selector' => '.site-description',
			'container_inclusive' => false,
			'render_callback' => 'Ember_customize_partial_blogdescription',
		) );
	}
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'Ember_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'Ember' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => Ember_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'Ember' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'Ember' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'Ember' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'Ember' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'Ember' );
}
add_action( 'customize_register', 'Ember_customize_register', 11 );

/**
 * Render the site title for the selective refresh partial.
 *
 * @since Twenty Fifteen 1.5
 * @see Ember_customize_register()
 *
 * @return void
 */
function Ember_customize_partial_blogname() {
	bloginfo( 'name' );
}
 ...
 * @return void
 */
function Ember_customize_partial_blogdescription() {
 ...
 *
 * Can be filtered with {@see 'Ember_color_schemes'}.
 *
 ...
 */
function Ember_get_color_schemes() {
	/**
	 ...
	 * }
	 */
	return apply_filters( 'Ember_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'Ember' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'Ember_get_color_scheme' ) ) :
/**
 ...
 */
function Ember_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = Ember_get_color_schemes();
...
endif; // Ember_get_color_scheme

if ( ! function_exists( 'Ember_get_color_scheme_choices' ) ) :
/**
 ...
 */
function Ember_get_color_scheme_choices() {
	$color_schemes                = Ember_get_color_schemes();
...
endif; // Ember_get_color_scheme_choices

if ( ! function_exists( 'Ember_sanitize_color_scheme' ) ) :
/**
 ...
 */
function Ember_sanitize_color_scheme( $value ) {
	$color_schemes = Ember_get_color_scheme_choices();
...
endif; // Ember_sanitize_color_scheme
 ...
 */
function Ember_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = Ember_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = Ember_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = Ember_get_color_scheme_css( $colors );

	wp_add_inline_style( 'Ember-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'Ember_color_scheme_css' );
 ...
 * @since Twenty Fifteen 1.0
 */
function Ember_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', Ember_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'Ember_customize_control_js' );
 ...
 * @since Twenty Fifteen 1.0
 */
function Ember_customize_preview_js() {
	wp_enqueue_script( 'Ember-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'Ember_customize_preview_js' );
 ...
 */
function Ember_get_color_scheme_css( $colors ) {
 ...
 * @since Twenty Fifteen 1.0
 */
function Ember_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-Ember-color-scheme">
		<?php echo Ember_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'Ember_color_scheme_css_template' );
